...
def test_derive_sam_cascade():
    from not_dot_net.backend.workflow_ad_account import derive_sam_candidates
...
def test_render_mail_uses_template():
    from not_dot_net.backend.workflow_ad_account import render_mail
    assert render_mail("{first}.{last}@x.y", "Alice", "Smith") == "alice.smith@x.y"


def test_generate_initial_password_meets_complexity():
    from not_dot_net.backend.workflow_ad_account import generate_initial_password
...
async def test_ad_account_creation_happy_submit(monkeypatch):
    ...
    result = await handle_ad_account_creation(request, form, ("admin", "pw"), actor)
...
async def test_ad_account_creation_rejects_existing_sam(monkeypatch):
    ...
    monkeypatch.setattr(wa, "ldap_lookup_by_sam",
...
async def test_ad_account_creation_group_failures_returned_not_raised(monkeypatch):
    ...
    monkeypatch.setattr(wa, "ldap_lookup_by_sam", lambda *a, **kw: None)
    monkeypatch.setattr(wa, "ldap_create_user",
                        lambda new, *a, **kw: f"CN=x,{new.ou_dn}")
    monkeypatch.setattr(wa, "ldap_add_to_groups",
                        lambda *a, **kw: {"CN=g,DC=x": "denied"})
    ...
    form = {"first_name": "A", "last_name": "S", "sam_account": "as",
            ...
            "groups": ["CN=g,DC=x"]}
    result = await handle_ad_account_creation(request, form, ("a", "p"), MagicMock())
...
async def test_ad_account_creation_ad_create_failure_keeps_uid(monkeypatch):
    ...
    monkeypatch.setattr(wa, "ldap_create_user", fail_create)
...
async def test_ad_account_creation_idempotent_retry_resets_password(monkeypatch):
    ...
    monkeypatch.setattr(wa, "ldap_create_user", _boom_create)

    def _reset(dn, pw, *a, **kw):
        reset_calls.append((dn, pw))
    monkeypatch.setattr(wa, "ldap_reset_password", _reset)
    ...
    result = await handle_ad_account_creation(request, form, ("admin", "pw"), MagicMock(id="a"))
